...
var indices = require('../../../../desarrollo-humano-data/data/json/desarrollo-humano.json');
...
function menu() {
	...
	nav.querySelector('.nav-item.general').addEventListener('click', function(event) {
		...
		// Switch the cantonal <select> back to 0 so that visually it doesn't look like a canton is selected.
		document.getElementById('viz-graph-select').selectedIndex = 0;
...
function canton() {
	...
	indexStat(2, indexStats.IDHc['2011'].Valor * 100);
	indexStat(3, indexStats.IEV['2011'].Valor * 100);
	indexStat(4, indexStats.IC['2011'].Valor * 100);
}
