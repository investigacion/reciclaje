...
function ranking() {
	var table, row, codes, codesLength;

	codes = Object.keys(garbage).filter(function(code) {

		// Filter out cantons with no data.
		// Test that every key being used has valid data.
		return ['tonsPerMonth', 'tonsRecoverablePerMonth', 'percentLocationsSeparate'].every(function(key) {
			return !isNaN(parseInt(garbage[code][key], 10));
		});
	});
	...
	row = function(tr, key) {
		...
		codes.sort(function(a, b) {
			...
			} else if (i === codesLength - 1) {
				td.classList.add('last');
			}

			position = codesLength - i;
...
function canton() {
	...
	var garbageStat = function(i, percentage) {
		var mercury = container.querySelector('.stat-1 .mercury');
...
function tip(code, position) {
	...
	html = '<div class="ranking">Ranking <br><span class="position">' + position + '°</span></div>';
